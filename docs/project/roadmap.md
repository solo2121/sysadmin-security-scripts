...
- [ ] Create Ansible roles to improve repeatability of DevOps lab provisioning, reducing reliance on Vagrant provisioners. Place roles under `labs/infrastructure/devops-linux-lab/ansible/` — the CI pipeline's `lint-ansible-roles` job already checks for that directory and will start running `ansible-lint` against it automatically once it exists, no CI changes needed.
