...
A full run of the Active Directory base lab (`vagrant up` through domain
promotion to a ready, health-checked domain controller):

| 1. Provider auto-detection | 2. AD promotion succeeds | 3. Health check + lab manager |
|---|---|---|
...
Recorded end-to-end with no cuts within each clip — `vagrant_manager.py`
resolving the provider and listing lab VMs, `dc01` completing AD promotion
and rebooting, and the post-boot health check confirming the domain
controller is reachable before handing back to the interactive menu.

A full run of the DevOps Linux lab (K3s bootstrap through an airgapped
Harbor registry to the DevSecOps attack-vector scenarios):

| 1. K3s node Ready | 2. Airgapped image seeding | 3. Attack vectors deploy clean |
|---|---|---|
| ![K3s node Ready and kubectl API ready, with Ingress NGINX installing via Helm](./assets/demos/devops1-01-k3s-ready-ingress.gif) | ![Container images being pulled, tagged, and pushed into the airgapped Harbor registry](./assets/demos/devops1-02-harbor-seeding.gif) | ![DevSecOps attack-vector scenarios (privileged escape, docker breakout, RBAC escalation, and more) deploying successfully, ending with Harbor login succeeding](./assets/demos/devops1-03-attack-vectors.gif) |

Same idea: `devops-1` bringing a K3s node to Ready, installing Ingress NGINX,
seeding an airgapped Harbor registry with every required image, then deploying
the intentionally-vulnerable DevSecOps attack-vector scenarios (privileged
escape, Docker breakout, RBAC escalation, a cryptominer cronjob, and more)
used to exercise Falco and the rest of the runtime-security stack.
...
|---|---|---|
| Devops-linux-lab (K3s/Harbor/DevSecOps) | ✅ `--provider=libvirt` | ✅ `--provider=virtualbox` |
| Active Directory lab (base) | ✅ `--provider=libvirt` | ✅ `--provider=virtualbox` |
| Active Directory lab (VLAN-segmented) | ✅ `--provider=libvirt` | ✅ `--provider=virtualbox` |
| CI validation | ✅ Vagrantfiles validated on every push via `vagrant-libvirt` plugin | ✅ Vagrantfiles validated on every push via GitHub-hosted VirtualBox |
| Networking | Auto-detected `vagrant-libvirt`/`default` libvirt network, falling back to `192.168.121.0/24` | Host-only network, defaults to `192.168.56.0/24` (override via `NETWORK_BASE`) |
| VLAN/internal segmentation | Native libvirt networks per VLAN | `virtualbox__intnet` per VLAN |
| Disk storage | qcow2 (`libvirt.storage :file`, `writeback` cache) | VDI (`vb.customize "createmedium" ... --format VDI`), attached via SATA controller |
| Nested virtualization | `cpu_mode = host-passthrough` (devops lab) / `host-passthrough` + `host` cpu model (AD labs) | `--nested-hw-virt on` / `--hwvirtex`, `--nestedpaging`, `--largepages on` |
| Graphics | VNC via `virtio` video (loopback only) | VMSVGA / VBoxSVGA controller, optional GUI via `LAB_GUI=true` |
| Linked clones | N/A (libvirt uses backing-file qcow2 by default) | Enabled (`vb.linked_clone = true`) to reduce disk usage |
| Guest additions | Not required | `vagrant-vbguest` auto-update supported |
| Best for | Linux hosts with nested virtualization; best performance, primary development target | Cross-platform hosts (Windows/macOS/Linux) without KVM/libvirt access |

This matrix reflects what's actually implemented in each lab's `Vagrantfile` — provider selection, storage, and networking logic are unified in a single file per lab rather than split into separate provider-specific Vagrantfiles.
