...
`devops-linux-lab` and `security/active-directory/base` each ship a single unified `Vagrantfile` supporting both KVM/libvirt and VirtualBox (select with `--provider`). `security/active-directory/vlan-segmented` still ships a separate VirtualBox-compatible `Vagrantfile` under its own `virtualbox/` subdirectory. See each lab's README for VirtualBox-specific setup, configuration, and troubleshooting.
...
For `devops-linux-lab` and `security/active-directory/base`, run
`vagrant up --provider=virtualbox` from the lab's own directory — same VMs,
IPs, and provisioning as libvirt, just a different provider block in the
same Vagrantfile. For `security/active-directory/vlan-segmented`, `cd` into
its `virtualbox/` subdirectory and run `vagrant up --provider=virtualbox`
instead. Requires [VirtualBox](https://www.virtualbox.org/) 7.0+ and
Vagrant >= 2.2 (the VirtualBox provider is built into Vagrant, no extra
plugin needed). Not supported on Apple Silicon/ARM hosts. See each lab's
README for details.
