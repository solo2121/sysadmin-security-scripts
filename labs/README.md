...
Run `vagrant up --provider=virtualbox` from the lab's own directory — same
VMs, IPs, and provisioning as libvirt, just a different provider block in
the same Vagrantfile. Requires [VirtualBox](https://www.virtualbox.org/)
7.0+ and Vagrant >= 2.2 (the VirtualBox provider is built into Vagrant, no
extra plugin needed). Not supported on Apple Silicon/ARM hosts. See each
lab's README for details.
